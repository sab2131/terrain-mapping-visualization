...
public class Spatial3DView extends View {
  ...
  public void drawView() {
    ...
    if (pts.size() > 1) {
        ...
        drawLineOnMap(scene, p1.x, p1.y, p2.x, p2.y);
        }
    }
    ArrayList<PVector> wps = model.getPath().getWaypoints();
    for (PVector wp: wps) {
        scene.fill(0, 0, 255);
        drawSphereOnMap(scene, wp.x, wp.y, 10);
    }
    PVector hover = model.getPath().getHoverPoint();
        if (hover != null) {
            scene.fill(255, 255, 0);
            drawBoxOnMap(scene, hover.x, hover.y, 10);
        }
